...
const STORAGE_KEYS = {
  ...
  UMD_BUILDINGS: "terpsync_umd_buildings_v10",
  UMD_CIRCLES: "terpsync_umd_circles_v10",
  UMD_SHUTTLES: "terpsync_umd_shuttles_v10",
  UMD_PARKING: "terpsync_umd_parking_v10",
  UMD_SAFETY: "terpsync_umd_safety_v10",
  UMD_SCAVENGER: "terpsync_umd_scavenger_v10",
...
export function loadCampusWeather(): CampusWeatherReport {
  const loaded = safeLoad<CampusWeatherReport>(STORAGE_KEYS.CAMPUS_WEATHER, initialUmdMainWeather);
  return {
    ...initialUmdMainWeather,
    ...(loaded || {}),
    hourlyForecast: Array.isArray(loaded?.hourlyForecast) && loaded.hourlyForecast.length > 0 ? loaded.hourlyForecast : initialUmdMainWeather.hourlyForecast,
    dailyForecast: Array.isArray(loaded?.dailyForecast) && loaded.dailyForecast.length > 0 ? loaded.dailyForecast : initialUmdMainWeather.dailyForecast,
    activeAlerts: Array.isArray(loaded?.activeAlerts) ? loaded.activeAlerts : initialUmdMainWeather.activeAlerts,
    clothingRecommendation: loaded?.clothingRecommendation || initialUmdMainWeather.clothingRecommendation,
    conditionIcon: loaded?.conditionIcon || initialUmdMainWeather.conditionIcon,
    radarImageUrl: loaded?.radarImageUrl || initialUmdMainWeather.radarImageUrl,
  };
}
...
// ─────────────────────────────────────────────────────────────
export function loadUmdBuildings(): UmdBuilding[] {
  const loaded = safeLoad<UmdBuilding[]>(STORAGE_KEYS.UMD_BUILDINGS, initialUmdBuildings);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdBuildings;
}
export function saveUmdBuildings(buildings: UmdBuilding[]): void {
  safeSave(STORAGE_KEYS.UMD_BUILDINGS, buildings);
}

export function loadUmdCircles(): LocationCircle[] {
  const loaded = safeLoad<LocationCircle[]>(STORAGE_KEYS.UMD_CIRCLES, initialUmdCircles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdCircles;
}
export function saveUmdCircles(circles: LocationCircle[]): void {
  safeSave(STORAGE_KEYS.UMD_CIRCLES, circles);
}

export function loadUmdShuttles(): UmdShuttle[] {
  const loaded = safeLoad<UmdShuttle[]>(STORAGE_KEYS.UMD_SHUTTLES, initialUmdShuttles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdShuttles;
}
export function saveUmdShuttles(shuttles: UmdShuttle[]): void {
  safeSave(STORAGE_KEYS.UMD_SHUTTLES, shuttles);
}

export function loadUmdParking(): UmdParkingGarage[] {
  const loaded = safeLoad<UmdParkingGarage[]>(STORAGE_KEYS.UMD_PARKING, initialUmdParking);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdParking;
}
export function saveUmdParking(parking: UmdParkingGarage[]): void {
  safeSave(STORAGE_KEYS.UMD_PARKING, parking);
}

export function loadUmdSafetyBeacons(): SafetyBeacon[] {
  const loaded = safeLoad<SafetyBeacon[]>(STORAGE_KEYS.UMD_SAFETY, initialUmdSafetyBeacons);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdSafetyBeacons;
}
export function saveUmdSafetyBeacons(beacons: SafetyBeacon[]): void {
  safeSave(STORAGE_KEYS.UMD_SAFETY, beacons);
}

export function loadUmdScavengerCheckpoints(): ScavengerHuntCheckpoint[] {
  const loaded = safeLoad<ScavengerHuntCheckpoint[]>(STORAGE_KEYS.UMD_SCAVENGER, initialUmdScavengerCheckpoints);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialUmdScavengerCheckpoints;
}
export function saveUmdScavengerCheckpoints(checkpoints: ScavengerHuntCheckpoint[]): void {
  safeSave(STORAGE_KEYS.UMD_SCAVENGER, checkpoints);
}
