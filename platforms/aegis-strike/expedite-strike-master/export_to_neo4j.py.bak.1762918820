...
Compatible with Neo4j 5.x (no illegal WITH-WHERE usage).
"""
...
# ---------------------------------------------------------------------
load_dotenv(dotenv_path=os.path.expanduser("~/vuln_intel/.env"))
...
if not NEO4J_PW:
    raise SystemExit("ERROR: NEO4J_PW not set. Put it in ~/.env or export it in your shell.")
...
# ---------------------------------------------------------------------
#  Neo4j write logic (Neo4j 5-compatible)
# ---------------------------------------------------------------------
def upsert_batch(tx, rows):
    """
    Upsert logic per row (Neo4j 5-compatible, no WHERE inside CALL).
    Creates/updates Asset, Vulnerability, and Source nodes, and
    HAS_VULNERABILITY / HAS_SCAN relationships.
    """
    cypher = """
    UNWIND $rows AS row
    ...
    // ---- Vulnerability (conditional merge) ----
    WITH a, row,
         CASE
    ...
    MERGE (a)-[:HAS_SCAN {import_date: row.import_date}]->(s)
    """
