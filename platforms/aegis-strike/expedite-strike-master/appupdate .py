...
# ======================================================================
# ======================================================================
#  TAB 8 — NEO4J GRAPH INTELLIGENCE (Dynamic + Categorized)
# ======================================================================
import dash_cytoscape as cyto
...
def neo4j_graphs_tab():
    return html.Div(
        [
            ...
            # TITLE
            # ==========================================================
            html.H3("🧠 Neo4j Graph Intelligence", className="text-warning mb-4"),
            ...
            cyto.Cytoscape(
                ...
                stylesheet=[
                    ...
                    # TYPE-BASED STYLES
                    {"selector": "node[type='Asset']", "style": {"background-color": "#00A8E8", "shape": "round-rectangle"}},
                    {"selector": "node[type='Service']", "style": {"background-color": "#007BFF", "shape": "hexagon"}},
                    ...
                    {"selector": "node[type='Exploit']", "style": {"background-color": "#8E44AD", "shape": "diamond"}},
                    {"selector": "node[type='ThreatActor']", "style": {"background-color": "#FF5733", "shape": "octagon"}},
            ...
            html.Div(
                [
                    ...
                    dcc.Dropdown(id="attack-start-node", placeholder="Start Node", style={"width": "50%", "marginBottom": "10px"}),
                    dcc.Dropdown(id="attack-target-node", placeholder="Target Node (Crown Jewel)", style={"width": "50%", "marginBottom": "10px"}),
                    html.Button("🚀 Trace Attack Path", id="attack-path-btn", className="btn btn-danger", n_clicks=0),
                ]
            ),
            ...
            html.Div(
                [
                    html.Button("📄 Export JSON", id="export-json-btn", className="btn btn-primary me-3"),
                    html.Button("📊 Export CSV", id="export-csv-btn", className="btn btn-success"),
...
def update_graph_types(category):
    if not category:
        raise PreventUpdate
    try:
        return [{"label": g, "value": g} for g in GRAPH_TYPES.get(category, [])]
    except:
        return []


# ==========================================================
# CALLBACK: Load Neo4j Graph (Main Graph Intelligence Engine)
# ==========================================================
@app.callback(
...
def update_neo4j_graph(graph_type):
    if not graph_type:
        return [], "Select a graph type"

    if graph_type not in GRAPH_QUERIES:
        return [], f"⚠ Unknown graph type: {graph_type}"

    query = GRAPH_QUERIES[graph_type]
                    ...
                    if hasattr(val, "id"):
                        ...
                        if nid not in nodes:
                            nodes[nid] = {
                                "data": {
                                    "id": nid,
                                    "label": val.get("name") or val.get("id"),
                                    "type": list(val.labels)[0]
                                }
                            }

                # Build edge if exists
    ...
    return elements, f"Showing: {graph_type}"


# ==========================================================
# CALLBACK: Node Info Panel
# ==========================================================
@app.callback(
...
def highlight_search(term, base_stylesheet):
    if not term:
        return base_stylesheet

    highlight = {
        "selector": f"[label *= '{term}']",
        "style": {
            "border-width": 4,
            "border-color": "yellow",
    ...
    return base_stylesheet + [highlight]
    ...
    [
        ...
        dbc.Tabs(
            ...
            children=[
                ...
                # 9. Attack Chain Simulator (NEW TAB)
                dbc.Tab(
