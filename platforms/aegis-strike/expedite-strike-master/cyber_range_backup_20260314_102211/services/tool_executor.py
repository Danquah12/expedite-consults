...
TOOL_COMMANDS = {
    ...
    "SSH Key Reuse":        "ssh -o StrictHostKeyChecking=no -o BatchMode=yes {target} id 2>&1",
...
def _build_command(tool_name: str, target: str, ssh_user: str, ssh_pass: str, wordlist: str) -> Optional[str]:
    ...
    if ssh_user and ssh_pass:
        ...
        ssh_auth_full = f"-o StrictHostKeyChecking=no -o ConnectTimeout=10 {ssh_user}@"
    else:
        ssh_prefix = ""
        ssh_auth_full = f"-o StrictHostKeyChecking=no -o ConnectTimeout=10 {ssh_user}@" if ssh_user else "-o StrictHostKeyChecking=no -o ConnectTimeout=10 "
