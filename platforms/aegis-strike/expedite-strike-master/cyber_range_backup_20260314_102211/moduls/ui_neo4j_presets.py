...
def update_import_feeds(source, min_cvss, kev_only, exploit_only):
    ...
    WHERE f.cve IS NOT NULL
      AND toFloat(f.cvss) >= $min_cvss
      AND ($source = '' OR toLower(f.source) = toLower($source))
    ...
    with driver.session() as session:
        ...
        for r in records:
            ...
            rows.append({
                ...
                "cvss": r["cvss"],
                "exploit_available": "true" if r["exploitable"] else "false",
                "in_kev": "true" if r["in_kev"] else "false",
            })
...
def load_neo4j_preset_graph(b1, b2, b3, b4, b5):
    """Executes preset architectural queries inside the unified portal."""
    clicked = ctx.triggered_id

    # Broad fallback queries — work regardless of exact relationship schema
    cypher_map = {
        # Any Host connected to anything
        "btn-assets":      """
            MATCH (a:Host)
            OPTIONAL MATCH (a)-[r]->(b)
            RETURN a, r, b LIMIT 120
        """,
        # Findings/Vulnerabilities and their hosts
        "btn-vuln":        """
            MATCH (f)
            WHERE f:Finding OR f:Vulnerability
            OPTIONAL MATCH (h)-[r]->(f)
            RETURN f, r, h LIMIT 120
        """,
        # Techniques → Tactics (direct or via any relationship)
        "btn-tech-tac":    """
            MATCH (t:Technique)
        ...
        """,
        # Any 2-hop path between hosts and findings
        "btn-attack-path": """
            MATCH p=(h:Host)-[*1..3]->(f)
            WHERE f:Finding OR f:Vulnerability OR f:Tactic
            RETURN p LIMIT 60
        """,
    }

    # Default query (on initial load or unknown trigger): show everything
