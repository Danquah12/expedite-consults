...
# -*- coding: utf-8 -*-
"""
 AI-Augmented Vulnerability Intelligence Dashboard
...
# ==========================================================
# CYBER RANGE — SERVICE BACKEND MODULES
# ==========================================================
...
# ==========================================================
# CYBER RANGE — UI MODULES (moduls folder)
# ==========================================================
...
# NEW — Neo4j ingestion engine
from cyber_range.services.neo4j_ingest import Neo4jIngestor
neo_ingest = Neo4jIngestor()

# ==========================================================
# CORE DASHBOARD TABS (already in your codebase)
# ==========================================================
from assessment import assessment_tab
from vulnerabilities import vulnerability_tab
from llm_module import llm_tab
from chatbot_module import chatbot_tab
from neo4j_graphs import neo4j_graphs_tab
from neo4j_preset_graphs import neo4j_preset_graphs_tab
from mitre_tab import mitre_tab_layout
from attack_chain_simulator import attack_chain_simulator_tab
from reporting_tab import reporting_tab
from voice_module import voice_tab
...
# ==========================================================
# ADVANCED ASSESSMENT TAB (UPLOAD → PARSE → ENRICH → VISUALIZE → AI)
# ==========================================================
def assessment_tab():

    return dbc.Container([

        html.H3("🧪 Advanced Security Assessment Suite",
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                html.H4("📁 Import Scan Files", className="text-info"),

                dcc.Upload(
                    id="assess-upload",
                    children=html.Div([
                        "📤 Drag & Drop or Click to Upload (Nmap XML, Nessus, OpenVAS)"
                    ]),
                    style={
                        "width": "100%",
                        "height": "80px",
                ...
                dbc.Button("🧩 Parse Scan Data", id="assess-parse-btn",
                           color="primary", className="mb-3 w-100"),

                dbc.Button("🔍 Enrich Vulnerabilities", id="assess-enrich-btn",
                           color="warning", className="mb-3 w-100"),

                dbc.Button("🕸 Map to Neo4j Graph", id="assess-graph-btn",
                           color="danger", className="mb-3 w-100"),

                html.Hr(),

                html.H4("📝 Scan Log", className="text-info"),
                html.Pre(id="assess-scan-log",
                         style={
                             "background": "#111",
                             "color": "#0f0",
                             "padding": "10px",
            ...
            dbc.Col([
                ...
                html.H4("📊 Visual Analytics", className="text-warning"),

                dcc.Graph(id="assess-cvss-heatmap",
                          style={"height": "300px"}),

                dcc.Graph(id="assess-severity-bar",
                          style={"height": "300px"}),

                dcc.Graph(id="assess-category-pie",
                          style={"height": "300px"}),

                dcc.Graph(id="assess-asset-vuln-graph",
                          style={"height": "300px"}),

            ], md=5),

            # ------------------------------------------------------------------
            # RIGHT COLUMN — AI EXECUTIVE ANALYSIS
            # ------------------------------------------------------------------
            dbc.Col([

                html.H4("🤖 AI Executive Summary", className="text-warning"),
                html.Div(id="assess-ai-summary",
                ...
                html.H4("🛠 AI Remediation Plan", className="text-warning"),
                html.Div(id="assess-ai-remediation",
                ...
                html.H4("⚔ AI Attack Path Findings", className="text-warning"),
                html.Div(id="assess-ai-attack-paths",
                ...
                dbc.Button(
                    ...
                    id="assess-ai-export-pdf",
                    color="secondary",
                    className="w-100 mt-2"
                ),

                dcc.Download(id="assess-ai-pdf"),
...
def register_all_callbacks(app):
    ...
    # -----------------------------
    # CYBER RANGE MODULE CALLBACKS
    # -----------------------------
    kc_callbacks(app)   # Kill Chain
    dt_callbacks(app)   # Digital Twin
    hp_callbacks(app)   # Honeypots / Deception
    exp_callbacks(app)  # Exploit AI
    wg_callbacks(app)   # Wargame AI (Red vs Blue)
    ti_callbacks(app)   # Threat Intelligence
    rm_callbacks(app)   # Remediation AI

    print("[+] All cyber-range callbacks registered successfully.")


# ==========================================================
# ADVANCED ASSESSMENT TAB — CALLBACK SUITE
# ==========================================================
...
# ==========================================================
# INTERNAL PARSE HELPERS (Nmap / Nessus / OpenVAS)
# ==========================================================

def parse_uploaded_file(contents, filename):
    """
    Detect file type and convert to dataframe.
    Supports: Nmap XML, Nessus XML, OpenVAS XML.
    """
    if contents is None:
    ...
    # Parse Nmap XML
    # -------------------------
    if filename.endswith(".xml") and "<nmaprun" in decoded.decode(errors="ignore"):
        try:
            import xml.etree.ElementTree as ET
            ...
            return df, "✅ Parsed Nmap scan successfully."
        except Exception as e:
            return None, f"❌ Error parsing Nmap XML: {e}"

    # -------------------------
    # Parse Nessus (.nessus XML)
    # -------------------------
    if filename.endswith(".nessus"):
        try:
            ...
            for item in tree.findall(".//ReportItem"):
                vulns.append({
                    "asset": item.attrib.get("ip", "unknown"),
                    "plugin": item.attrib.get("pluginID"),
                    "name": item.attrib.get("pluginName"),
                    "severity": item.attrib.get("severity"),
                    "cve": item.attrib.get("cve", "None"),
                })
            df = pd.DataFrame(vulns)
            return df, "✅ Parsed Nessus scan successfully."
        except Exception as e:
            return None, f"❌ Error parsing Nessus file: {e}"

    # -------------------------
    # Parse OpenVAS (.xml)
    # -------------------------
    if filename.endswith(".xml") and "<report" in decoded.decode(errors="ignore"):
        try:
            import xml.etree.ElementTree as ET
            tree = ET.fromstring(decoded)
            vulns = []
            for item in tree.findall(".//result"):
                vulns.append({
                    "asset": item.findtext("host", "unknown"),
                    "name": item.findtext("name", "unknown"),
                    "severity": item.findtext("threat", "unknown"),
                    "cve": item.findtext("nvt/cve", "None"),
                })
            df = pd.DataFrame(vulns)
            return df, "✅ Parsed OpenVAS scan successfully."
        except Exception as e:
            return None, f"❌ Error parsing OpenVAS XML: {e}"

    return None, "❌ Unsupported file type."


# ==========================================================
# CALLBACK: PARSE UPLOADED FILE
# ==========================================================
@app.callback(
    Output("assess-scan-log", "children"),
    Output("assess-cvss-heatmap", "figure"),
    Output("assess-severity-bar", "figure"),
    Output("assess-category-pie", "figure"),
    Output("assess-asset-vuln-graph", "figure"),
    Input("assess-parse-btn", "n_clicks"),
    State("assess-upload", "contents"),
    State("assess-upload", "filename"),
    prevent_initial_call=True
)
def parse_assessment_file(n, contents, filename):
    ...
    # Save to global store dynamically
    global ASSESS_DF
    ASSESS_DF = df.copy()

    # ----------------------- Generate Graphs -----------------------

    # Heatmap (severity vs asset)
    if "severity" in df.columns and "asset" in df.columns:
        heatmap = px.density_heatmap(df, x="severity", y="asset",
                                     title="CVSS / Severity Heatmap")
    else:
        heatmap = {}

    # Severity Bar Chart
    if "severity" in df.columns:
        bar = px.histogram(df, x="severity", title="Severity Histogram")
    else:
        bar = {}

    # Category Pie (by service or plugin/type)
    if "service" in df.columns:
        pie = px.pie(df, names="service", title="Service Distribution")
    elif "name" in df.columns:
        pie = px.pie(df, names="name", title="Vulnerability Types")
    else:
        pie = {}

    # Asset-vuln Graph
    if "asset" in df.columns:
        agg = df.groupby("asset").size().reset_index(name="count")
        graph = px.bar(agg, x="asset", y="count", title="Vulnerabilities per Asset")
    else:
        graph = {}
...
# ==========================================================
# CALLBACK: AI EXECUTIVE SUMMARY, ATTACK PATHS, REMEDIATION
# ==========================================================
@app.callback(
    Output("assess-ai-summary", "children"),
    Output("assess-ai-remediation", "children"),
    Output("assess-ai-attack-paths", "children"),
    Input("assess-enrich-btn", "n_clicks"),
    prevent_initial_call=True
)
def ai_generate_summary(n):

    global ASSESS_DF

    if "ASSESS_DF" not in globals():
        return "❌ No parsed data available.", "", ""

    df = ASSESS_DF.copy()

    vulns_text = df.to_string()

    prompt = f"""
You are a cybersecurity analyst. Generate:

1. An executive summary of the vulnerabilities.
2. Key attack paths.
...
# ==========================================================
# CALLBACK: EXPORT AI PDF REPORT
# ==========================================================
@app.callback(
    Output("assess-ai-pdf", "data"),
    Input("assess-ai-export-pdf", "n_clicks"),
    State("assess-ai-summary", "children"),
    State("assess-ai-remediation", "children"),
    State("assess-ai-attack-paths", "children"),
    prevent_initial_call=True
)
def export_ai_pdf(n, summary, remediation, paths):

    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=letter)
    text = c.beginText(30, 750)
    text.setFont("Helvetica", 10)

    text.textLine("AI Security Assessment Report")
    text.textLine("------------------------------------")
    text.textLine(f"Generated: {datetime.now()}\n")

    text.textLine("Executive Summary:")
    text.textLine(summary.replace("\n", " "))

    text.textLine("\nRemediation Plan:")
    text.textLine(remediation.replace("\n", " "))

    text.textLine("\nAttack Path Findings:")
    text.textLine(paths.replace("\n", " "))
    ...
    return dict(
        content=pdf_bytes,
        filename="Assessment_AI_Report.pdf",
        type="application/pdf"
    )


@app.callback(
    Output("assess-scan-log", "children", allow_duplicate=True),
    Input("assess-graph-btn", "n_clicks"),
    prevent_initial_call=True
)
def ingest_into_neo4j(n):
    global ASSESS_DF
    ...
    try:
        neo_ingest.ingest_dataframe(ASSESS_DF)
        return "✅ Successfully pushed parsed vulnerabilities into Neo4j graph."
    except Exception as e:
        return f"❌ Neo4j ingestion error: {e}"
    ...
    [
        ...
        dbc.Tabs(
            ...
            children=[
                ...
                # =====================================================
                # 10. CYBER-RANGE ADVANCED MODULES (NEW TABS)
                # =====================================================

                # 10. Kill Chain Generator
                dbc.Tab(
                    killchain_tab(),
                    label="Kill Chain",
                    tab_id="killchain"
                ...
                # 15. Threat Intelligence (KEV, EPSS, AI)
                dbc.Tab(
                    ti_feed_tab(),
                ...
                # 16. Automated Remediation & Hardening AI
                dbc.Tab(
                    remediation_tab(),
                    label="Remediation AI",
                    tab_id="rem_ai"
                ),
