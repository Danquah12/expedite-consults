...
@app.callback(
    ...
    Input("acs-run", "n_clicks"),
    [
        State("acs-start", "value"),
        State("acs-target", "value")
    ],
    prevent_initial_call=True
)
def run_attack_chain(n, start, target):
    if not start or not target:
        return "⚠️ Select both start and target nodes."
    ...
    try:
        ...
        with driver.session() as session:
            q = """
            MATCH p = shortestPath(
                (start {id:$start})-[:RUNS_SERVICE|HAS_VULNERABILITY|MAPS_TO_TECHNIQUE*..6]->(target {id:$target})
            )
            RETURN p
            """
            result = session.run(q, start=start, target=target).single()
            ...
            chain = []
            for node in path.nodes:
                chain.append(f"{list(node.labels)[0]} — {node.get('id') or node.get('name')}")
    ...
    [
        ...
        dbc.Tabs(
            ...
            children=[
                ...
                # 7. MITRE ATT&CK Intelligence
                dbc.Tab(mitre_tab_layout(), label="MITRE ATT&CK Intelligence", tab_id="mitre"),

                # 8. Attack Chain Simulator (NEW TAB)
                dbc.Tab(attack_chain_simulator_tab(), label="Attack Chain Simulator", tab_id="attack_sim"),
