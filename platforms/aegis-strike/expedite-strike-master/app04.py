...
# ==========================================================
# KILL CHAIN — FIXED CALLBACKS (Separate Tab Sync Mode)
# ==========================================================
...
# ----------------------------------------------------------
# Populate Start/Target dropdowns
# ----------------------------------------------------------
@app.callback(
    [
        Output("attack-start-node", "options"),
...
# ----------------------------------------------------------
# GENERATE KILL CHAIN (outputs to BOTH tabs)
# ----------------------------------------------------------
@app.callback(
    [
        Output("killchain-content", "children"),        # Attack Chain Simulator
        Output("killchain-tab-output", "children")      # Kill Chain Tab
    ],
    Input("generate-killchain", "n_clicks"),
    [
...
def update_killchain(n_clicks, start_node, target_node):
    if not n_clicks:
        raise PreventUpdate

    if not start_node or not target_node:
        msg = html.Div("❗ Select both START and TARGET nodes.", className="text-danger")
        return msg, msg

    try:
        kc = KillChainGenerator().generate_chain(start_node, target_node)
        rendered = killchain_tab(kc)
        return rendered, rendered   # sync both tabs

    except Exception as e:
        msg = html.Div(f"[Kill Chain Error] {str(e)}", className="text-danger")
        return msg, msg
