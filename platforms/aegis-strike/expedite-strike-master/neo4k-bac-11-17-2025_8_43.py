...
# ==========================================================
#  MAP CPEs → CVEs (NVD Lookups)
# ==========================================================
def create_vulns_from_cpe(limit=10):
    """
    Query NVD API for each unique CPE and create Vulnerability nodes (CVE IDs).
    """
    ...
    with driver.session() as session:
        ...
        for cpe in cpes:
            try:
                ...
                resp = requests.get(f"{NVD_API}?cpeName={cpe}", timeout=20)
                ...
                logger.warning(f"⚠️ Error fetching CVEs for {cpe}: {e}")
...
def enrich_cves_from_nvd(limit=50):
    ...
    This now includes automatic creation of vulnerabilities if missing.
    """
    ...
    with driver.session() as session:
        # Get existing vulnerabilities needing updates
        q = """
...
def auto_link_vulnerabilities():
    ...
    with driver.session() as session:
        ...
        MERGE (s)-[:HAS_VULNERABILITY {linked_at: datetime()}]->(v)
