...
def render_tabs(active_tab):
    ...
    if active_tab == "neo4j_presets":
        return neo4j_preset_graphs_tab()

    if active_tab == "mitre":
        return mitre_tab_layout()

    if active_tab == "attack_sim":
        return attack_chain_simulator_tab()

    if active_tab == "killchain":
        return killchain_tab()

    if active_tab == "digital_twin":
        return digital_twin_tab()

    if active_tab == "honeypots":
        return honeypot_tab()

    if active_tab == "exploit_ai":
        return exploit_ai_tab()

    if active_tab == "wargame":
        return wargame_tab()

    if active_tab == "ti_feed":
        return ti_feed_tab()

    if active_tab == "rem_ai":
        return remediation_tab()
