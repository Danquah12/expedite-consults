...
def generate_pdf(results: dict, output_path: str) -> str:
    ...
    def bg_page(canvas, doc):
        ...
        if doc.page > 1:
            ...
            canvas.setFillColor(rc(C_TEAL))
            canvas.drawString(18*mm, H-7*mm, f"AEGIS  |  External Pentest Report  |  {target[:50]}")
    ...
    # COVER PAGE
    # ════════════════════════════════════════════════════════════════
    story += [SP(20), P("AEGIS SECURITY", sTitle), SP(2),
    ...
    story.append(P(f"Aegis Security conducted an external penetration test against <b>{target}</b> "
    ...
    if not all_f:
        ...
        if len(all_f) > 50:
            story.append(P(f"NOTE: {len(all_f)-50} additional findings available in the Aegis platform.", sSmall))
...
def _html_fallback(results, output_path, err):
    ...
    with open(html_path,"w") as f:
        f.write(f"<html><body><h1>Aegis Pentest Report</h1><p>PDF library error: {err}</p>"
