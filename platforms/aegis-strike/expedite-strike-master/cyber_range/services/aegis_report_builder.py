"""
aegis_report_builder.py  —  AEGIS Professional Pentest Report Generator
=========================================================================
...
def generate_full_report(data: dict, output_path: str, exploit_evidence: list = None) -> str:
    ...
    def bg_page(canvas, doc):
        ...
        canvas.drawString(20*mm, 8*mm,
            f"AEGIS Security  ·  CONFIDENTIAL  ·  Page {doc.page}")
    ...
    tools_list = [
        ...
        ("AEGIS Engine","Automated exploitation and evidence collection"),
    ]
    ...
    for fi, f in enumerate(sorted_findings, 1):
        ...
        detail_rows = [
            kv_row("Finding ID:",    f"AEGIS-EXT-{fi:03d}"),
            ...
            kv_row("Evidence:",      f"Confirmed by {f.get('tool','AEGIS')} scanner. "
                   f"HTTP response analysis and manual verification performed."),
            kv_row("Exploitation:",  "Auto-exploited by AEGIS Engine. See Appendix B for PoC."
    ...
    story += [SP(1)]
    story.append(P(
        "The AEGIS auto-exploitation engine confirmed that multiple vulnerabilities are "
    ...
    story += [
        ...
        P("CONFIDENTIAL — For authorized recipients only. This report must not be "
          "reproduced or distributed without written consent of AEGIS Security.", sConf),
        SP(1),
        P(f"Report generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}  ·  "
          f"AEGIS Pentest Engine v2  ·  PTES / NIST SP 800-115 Aligned", sConf),
    ]
