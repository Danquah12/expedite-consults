"""
email_reporter.py  —  AEGIS Report Email Delivery System
=========================================================
...
def load_config() -> dict:
    ...
    return {
        ...
        "username":  "",
        "password":  "",
        "from_name": "AEGIS Pentest Engine",
        "from_addr": "",
        "recipients": [],
        "cc":        [],
    }
...
<div class="wrapper">
  ...
  <div class="footer">
    ...
    <p style="margin-top:12px;">Generated by AEGIS Pentest Engine v2 · PTES / NIST SP 800-115 Aligned<br>
...
def send_report(pdf_path: str, cfg: dict, report_meta: dict = None) -> tuple[bool, str]:
    ...
    msg["From"]    = f"{cfg.get('from_name','AEGIS Pentest Engine')} <{cfg.get('from_addr') or cfg['username']}>"
    msg["To"]      = ", ".join(cfg["recipients"])
    if cfg.get("cc"):
        msg["Cc"]  = ", ".join(cfg["cc"])
    msg["Subject"] = (f"[AEGIS] External Penetration Test Report — {posture} RISK — "
                      f"{date_str} — {targets[:40]}")
    msg["X-Priority"]   = "1" if posture in ("CRITICAL","HIGH") else "3"
    msg["X-AEGIS-Tool"] = "AEGIS-Pentest-Engine/2.0"

    # Plain text fallback
    plain = (
        f"AEGIS EXTERNAL PENETRATION TEST REPORT\n"
        f"{'='*50}\n\n"
        ...
        f"AEGIS Pentest Engine v2 — PTES/NIST SP 800-115\n"
    )
