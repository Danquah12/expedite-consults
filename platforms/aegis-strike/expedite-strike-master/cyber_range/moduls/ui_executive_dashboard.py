...
def _neo4j_scanner_findings(limit: int = 50) -> list[dict]:
    ...
    try:
        ...
        print(f"[exec-dash scanner-findings] Neo4j offline — using mock data")

    # Fallback: use mock findings
    try:
        from cyber_range.services.neo4j_mock import get_mock_findings
        results = []
        for f in get_mock_findings()[:limit]:
            module = _SOURCE_MODULE.get(f.get("source", ""), f.get("scanner", "Unknown").title())
            results.append({
                "module":     module,
...
def _neo4j_severity_counts() -> dict:
    ...
    try:
        ...
        print(f"[neo4j-sev-counts] Neo4j offline — using mock data")

    # Fallback: use mock graph findings
    try:
        from cyber_range.services.neo4j_mock import get_mock_findings
        for f in get_mock_findings():
...
def _build_trend(trend_data: list[dict]) -> go.Figure:
    if not trend_data:
        # Auto-generate trend data from available sources
        try:
    ...
    if not trend_data:
        # Try Neo4j severity counts (with mock fallback)
        try:
    ...
    if not trend_data:
        # Last resort: use mock findings directly
        try:
            from cyber_range.services.neo4j_mock import get_mock_findings
            mf = get_mock_findings()
            c = sum(1 for f in mf if f.get("severity") == "Critical")
            h = sum(1 for f in mf if f.get("severity") == "High")
            m = sum(1 for f in mf if f.get("severity") == "Medium")
            l = sum(1 for f in mf if f.get("severity") in ("Low", "Info"))
            if c + h + m + l > 0:
                trend_data = _synthesize_trend(c, h, m, l)
        except Exception:
            pass

    if not trend_data:
        # Absolute fallback: empty placeholder
        fig = go.Figure()
        fig.update_layout(
...
def _build_module_bar(module_stats: dict) -> go.Figure:
    if not module_stats:
        # Auto-generate from available data sources
        try:
            from cyber_range.services.neo4j_mock import get_mock_findings
            _SOURCE_MODULE_LOCAL = {
                "nmap": "Nmap", "zap": "ZAP", "nuclei": "Nuclei",
                "burp": "Burp Suite", "openvas": "OpenVAS",
                "nikto": "Nikto", "pentest-engine": "Pentest Engine",
                "ssl-check": "SSL Check", "aegisprobe": "AegisProbe",
            }
            module_stats = {}
            for f in get_mock_findings():
                src = f.get("source", f.get("scanner", "Unknown"))
                m = _SOURCE_MODULE_LOCAL.get(src, src.title())
                if m not in module_stats:
                    module_stats[m] = {"Critical": 0, "High": 0, "Medium": 0, "Low": 0}
                sev = f.get("severity", "Medium")
                if sev in module_stats[m]:
                    module_stats[m][sev] += 1
        except Exception:
            pass
                ...
                dbc.Col([
                    ...
                    *[html.Div([
                    ...
                    html.Div([
                        ...
                        html.Span([
                            html.Span("🔍 View Threat Intel", style={
                                "color": "#00aadd", "fontSize": "12px",
                                "fontWeight": "700", "cursor": "pointer",
                            }),
                            html.Span(" →", style={"color": "#00aadd"}),
                        ]),
                    ...
                    html.Div([
                        html.Img(
                            src="data:image/svg+xml;utf8," + (
                                ...
                                "<text x='34' y='52' text-anchor='middle' fill='%2300c88c' font-size='11' font-weight='bold' font-family='monospace'>AEGIS</text>"
