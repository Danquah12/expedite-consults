# ui_killchain.py — COMPLETE, FIXED, FULL VERSION
...
# ==========================================================
# DIRECT NEO4J NODE LOADER (No Cytoscape dependency)
# ==========================================================
db = Neo4jEngine()

def load_nodes_direct():
    """Load all Neo4j nodes directly for dropdown population."""
    try:
        q = """
        MATCH (n)
        ...
        print(f"[KillChain] Error loading nodes: {e}")
        return []
...
def render_kill_chain(chain):
    ...
    if mitre_map:
        mitre_html = html.Ul([
            html.Li([html.B(tactic + ": "), html.Span(", ".join(techs))])
            for tactic, techs in mitre_map.items()
        ])
    else:
        mitre_html = html.Div("No MITRE techniques mapped.", className="text-muted")
...
def generate_kill_chain_pdf(chain):
    ...
    # MITRE
    pdf.setFont("Helvetica-Bold", 14)
    pdf.drawString(50, y, "MITRE Techniques:")
    y -= 30

    mitre_map = {}
    for entry in chain["MITRE_Chain"]:
        t = entry.get("tactic")
        tech = entry.get("technique")
        if t and tech:
            mitre_map.setdefault(t, []).append(tech)
    ...
    pdf.setFont("Helvetica", 12)
    for tactic, techs in mitre_map.items():
        pdf.drawString(50, y, f"{tactic}: {', '.join(techs)}")
        y -= 20
        if y < 60:
            pdf.showPage() ; y = 800
    ...
    pdf.setFont("Helvetica", 12)
    for line in wrap_text(chain["Narrative"], 95):
...
def register_callbacks(app):
    ...
    # Populate dropdowns (direct Neo4j query)
    @app.callback(
        [
    ...
    # Run Kill Chain
    @app.callback(
        Output("killchain-output", "children", allow_duplicate=True),
    ...
    print("[ui_killchain] Loaded successfully with DIRECT Neo4j node loader.")
