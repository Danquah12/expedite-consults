...
from cyber_range.moduls.ui_killchain import register_callbacks as kc_callbacks
...
def attack_chain_simulator_layout():
    return dbc.Container(
        [
            html.H2("⚔️ Attack Chain Simulator", className="text-warning mt-4"),
            ...
            html.Div(id="attack-chain-narrative", className="text-light mt-4")
        ]
    )

@app.callback(
    [
        Output("attack-start-node", "options", allow_duplicate=True),
        Output("attack-target-node", "options", allow_duplicate=True),
    ],
    Input("tabs", "active_tab"),
    prevent_initial_call=False
)
def load_attack_chain_nodes(active_tab):
    if active_tab != "attack_chain":
        raise dash.exceptions.PreventUpdate
    ...
    try:
        with driver.session() as session:
            result = session.run("""
                ...
                ORDER BY name
            """)
            nodes = [{"label": r["name"], "value": r["name"]} for r in result]

        return nodes, nodes

    except Exception as e:
        print(f"[AttackChain] Error loading nodes: {e}")
        return [], []
...
@app.callback(
    [
        Output("attack-chain-graph", "elements", allow_duplicate=True),
        Output("attack-chain-narrative", "children", allow_duplicate=True)
    ],
...
def simulate_attack_chain(_, start_node, target_node):
    if not start_node or not target_node:
        return [], "❗ Select both a start and target node."
    ...
    try:
        with driver.session() as session:
            result = session.run(query, start=start_node, target=target_node)
            record = result.single()

            if not record:
                return [], f"⚠ No attack path found between **{start_node}** and **{target_node}**."
            ...
            for node in path.nodes:
                elements.append({
                    "data": {"id": node.id, "label": node.get("name")}
                })
        ...
        **Hops:** {len(path.nodes)-1}  
        ...
        return elements, narrative

    except Exception as e:
        print(f"[AttackChain] Path error: {e}")
        return [], f"❌ Error: {e}"
...
# ==========================================================
# FIXED CALLBACK — Populate Kill Chain dropdowns automatically
# ==========================================================
@app.callback(
    [
        Output("killchain-start-node", "options", allow_duplicate=True),
        Output("killchain-target-node", "options", allow_duplicate=True),
    ],
    Input("neo4j-graph", "elements"),
    prevent_initial_call=True
)
def populate_dropdowns(elements):
    if not elements:
        return [], []
    ...
    for el in elements:
        if "data" in el and "id" in el["data"]:
            nid = el["data"]["id"]
            label = el["data"].get("label", str(nid))
            if nid not in seen:
                seen.add(nid)
                options.append({"label": label, "value": nid})
