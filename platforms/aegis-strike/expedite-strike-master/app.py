...
"""
Ægis Security Operations Center — AI-Augmented Cyber Defense Platform
...
_LANDING_HTML_PATH = "/opt/vuln_intel/app/templates/landing.html"

def _serve_landing():
    with open(_LANDING_HTML_PATH, "r") as _f:
...
_PERMS_FILE = "/opt/vuln_intel/app/user_permissions.json"
...
<body><div class="box">
  ...
  <a href="/app/">↩ Return to Ægis</a>
    ...
    try:
        ...
        # ── Try enterprise PDF report (CVSS/compliance/financial/remediation) ──
        ts   = _dt_rpt.now().strftime("%Y%m%d_%H%M%S")
        tgt  = _tgt_label.replace(".", "_").replace("/","_")[:30]
        fname = f"pentest_{tgt}_{ts}.pdf"

        if _exploit_db_data and _exploit_db_data.get("results"):
            # Use enterprise report generator with full enrichment
            try:
                from cyber_range.exploit_db.report_generator import (
                    generate_executive_report, generate_technical_report,
                )
                ...
                report_type = _freq2.args.get("type", "executive")
                narrative = ""
                enterprise = _exploit_db_data.get("enterprise", {})
                if isinstance(enterprise, dict):
                    narrative = enterprise.get("attack_narrative", "")

                if report_type == "technical":
                    generate_technical_report(
                        _exploit_db_data, _tgt_label,
                        narrative=narrative, output_path=out_path,
                        full_scan=scan)
                else:
                    generate_executive_report(
                        _exploit_db_data, _tgt_label, output_path=out_path,
                        full_scan=scan)

                print(f"[PENTEST PDF] Enterprise report generated: {out_path}")
                return send_file(out_path, mimetype="application/pdf",
                                as_attachment=True, download_name=fname)
            except Exception as _ent_err:
                print(f"[PENTEST PDF] Enterprise report failed: {_ent_err}, "
                      "falling back to legacy generator")

        # ── Fallback: legacy PDF generator ────────────────────────────────────
        # ── Debug: dump what we have ──────────────────────────────────
        print(f"[PENTEST PDF] === REPORT DEBUG ===")
        print(f"[PENTEST PDF] scan_id={scan_id}")
        print(f"[PENTEST PDF] targets={_tgts}")
        print(f"[PENTEST PDF] raw_results keys: {list(_raw_results.keys())}")
        for _rk, _rv in _raw_results.items():
            if isinstance(_rv, dict):
                print(f"[PENTEST PDF]   {_rk}: phases={list(_rv.keys())}")
                for _pk, _pv in _rv.items():
                    if isinstance(_pv, list):
                        for _pi in _pv:
                            _pt = _pi.get("tool","?") if isinstance(_pi, dict) else "?"
                            _pd = _pi.get("data",{}) if isinstance(_pi, dict) else {}
                            _pf = _pd.get("findings",[])
                            _pe = _pd.get("error","")
                            print(f"[PENTEST PDF]     {_pk}/{_pt}: findings={len(_pf)}, error={str(_pe)[:60]}")
            else:
                print(f"[PENTEST PDF]   {_rk}: type={type(_rv).__name__}")
        print(f"[PENTEST PDF] flat_data tools: {list(_flat_data.keys())}")
        for _fk, _fv in _flat_data.items():
            if isinstance(_fv, dict):
                _ff = _fv.get("findings", [])
                _fe = _fv.get("error", "")
                print(f"[PENTEST PDF]   {_fk}: findings={len(_ff)}, error={str(_fe)[:60]}")
            else:
                print(f"[PENTEST PDF]   {_fk}: type={type(_fv).__name__}")
        print(f"[PENTEST PDF] exploit_db_data: {'YES' if _exploit_db_data else 'NO'}")
        print(f"[PENTEST PDF] === END DEBUG ===")

        results = {
            "target":     _tgt_label,
            "targets":    _tgts,
            "timestamp":  _dt_rpt.now().strftime("%Y-%m-%d %H:%M"),
            "phases_run": scan.get("phases", ["passive","active","vuln"]),
            "data":       _flat_data,
        }
        out_path = _os_rpt.path.join(_tmp.gettempdir(), fname)
        from cyber_range.services.pt_pdf_generator import generate_pdf
        final_path = generate_pdf(results, out_path)
        mime = "application/pdf"
        if final_path.endswith(".html"):
            mime  = "text/html"
            fname = fname.replace(".pdf", ".html")
        return send_file(final_path, mimetype=mime,
                         as_attachment=True, download_name=fname)
...
def render_tab(active_tab, pipeline_data):
    ...
    # ── Auth gate: username + password only — no module permissions ──────
...
def _intercept_admin_final():
    ...
    # ── Gate: / and /landing ALWAYS show the landing page (auth is handled by /login) ──
    if path in ('', '/landing'):
        with open('/opt/vuln_intel/app/templates/landing.html') as _lf:
            return _lf.read(), 200, {'Content-Type': 'text/html; charset=utf-8'}
...
def update_neo4j_graph_health(n_intervals, active_tab):
    ...
    # Only update when System Health tab is active
    if active_tab != "system":
        raise PreventUpdate
    ...
    try:
        with neo4j_driver.session() as session:
            ...
            assets = session.run(
                "MATCH (a:Host) RETURN count(a) AS cnt"
            ).single()["cnt"]
            ...
            findings = session.run(
                "MATCH (f:Finding) RETURN count(f) AS cnt"
            ).single()["cnt"]
        ...
        # simulated sub/svc progression based on port prog
        sub_p = min(100, prog * 1.5) if prog > 0 else 0
        port_p = prog
        svc_p = min(100, prog * 0.8) if prog > 0 else 0
        ...
        if SCAN_STATE.get("running"):
            ...
            sub_p, port_p, svc_p = 100, 100, 100 # assume done if not running
...
def assessment_export_ptes(n):
    ...
    if not report_path:
        return dash.no_update, dbc.Alert(
            "⚠ No scan report found. Run an AegisProbe scan first.",
...
def update_assessment_metrics(active_tab, _, scan_intervals):
    ...
    try:
        ...
        print("[Assessment Neo4j ERROR]", e)
...
def _build_mega_nav():
    ...
    attack = _group("🎯  ATTACK SURFACE", "attack-nav-dropdown", [
        ...
        _col("MITRE ATT&amp;CK &amp; Simulation", [
            ...
            _divider(),
            _item("🗂️", "MITRE ATT&amp;CK Intel","Technique library & threat mapping",           "menu-mitre-0"),
        ...
        _cta_col([
            _cta("View Attack Graph", "🕸️", "menu-neo4j-cta"),
            _divider(),
    ...
    redteam = _group("🔴  RED TEAM", "redteam-nav-dropdown", [
        _col("Web App Assessment", [
            _sec("🌐 WEB APP ASSESSMENT"),
            _item("🔍", "Recon / Scan",          "Active web discovery & fingerprinting",         "menu-waa-0"),
            _item("💥", "Exploitation",           "XSS, SQLi, SSRF & injection attacks",           "menu-waa-1"),
            _item("🔑", "Privilege Escalation",   "Vertical/horizontal privilege abuse",           "menu-waa-2"),
            _item("🔀", "Lateral Movement",       "Cross-domain pivot & session hijack",           "menu-waa-3"),
            _item("🕵️", "Post Exploitation",     "Persistence, exfil & C2 simulation",            "menu-waa-4"),
            _item("📄", "Reporting",              "Findings export & evidence packaging",           "menu-waa-5"),
        ]),
        _col("Automated PT &amp; Active Directory", [
            _item("🤖", "Automated Pen Testing",  "Full-pipeline autonomous PT engine",            "menu-apt-0",         primary=True),
            _item("💣", "Exploitation Engine",    "Metasploit-correlated exploit runner",          "menu-apt-1"),
            _item("🎭", "Post Exploitation",      "Automated C2 & lateral movement chains",        "menu-apt-2"),
            _divider(),
            _item("🏰", "AD RedOps Scanner",      "BloodHound-powered AD attack path analysis",   "menu-ad-0",          primary=True),
            _item("🏗️", "Red Team Infrastructure","C2 server & implant management",               "menu-redteam-0"),
            _item("🏰", "PingCastle Assessment",  "Windows AD security posture scoring",           "menu-pingcastle-0"),
            _divider(),
            _item("🔑", "IAM / MFA Bypass",       "Identity & auth bypass simulation",             "menu-iam-redteam-0"),
            _item("🧠", "LLM Red Teaming",         "Jailbreak & prompt injection testing",         "menu-llm-redteam-0"),
        ]),
        _cta_col([
            _cta("Start Pentest", "🎯", "menu-apt-cta"),
            _divider(),
            _item("🎤", "Voice Attack Simulator", "Vishing & social engineering AI",               "menu-voice-0"),
    ...
    return html.Div(
        [home, platform, recon, attack, redteam, defence, specialised, grc, dfir, reports],
...
app.layout = dbc.Container(
    [
        ...
        html.Div([
            html.H2("Ægis Security Operations Center",
...
@callback(
    Output("tabs", "active_tab", allow_duplicate=True),
    [
        Input("menu-apt-0", "n_clicks"),
        Input("menu-apt-1", "n_clicks"),
        Input("menu-apt-2", "n_clicks"),
    ],
    prevent_initial_call=True
)
def route_apt_dropdowns(*args):
    ctx = dash.callback_context
    if not ctx.triggered:
        raise dash.exceptions.PreventUpdate
    ...
    mapping = {
        "menu-apt-0": "automated_pt",
        "menu-apt-1": "exploitation",
        "menu-apt-2": "post_exploitation",
    }
...
@callback(
    Output("tabs", "active_tab", allow_duplicate=True),
    [
        Input("menu-waa-0", "n_clicks"),   # Recon / Scan
        Input("menu-waa-1", "n_clicks"),   # Exploitation
        Input("menu-waa-2", "n_clicks"),   # Privilege Escalation
        Input("menu-waa-3", "n_clicks"),   # Lateral Movement
        Input("menu-waa-4", "n_clicks"),   # Post Exploitation
        Input("menu-waa-5", "n_clicks"),   # Reporting
    ],
    prevent_initial_call=True
)
def route_web_app_assessment(*args):
    ctx = dash.callback_context
    if not ctx.triggered:
        raise dash.exceptions.PreventUpdate
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    mapping = {
        "menu-waa-0": "unified_scan",       # Recon / Scan → Unified Scan Engine
        "menu-waa-1": "exploitation",        # Exploitation
        "menu-waa-2": "privesc",             # Privilege Escalation → PT dashboard
        "menu-waa-3": "killchain",           # Lateral Movement → Kill Chain map
        "menu-waa-4": "post_exploitation",   # Post Exploitation → Premium PostEx Command Center
        "menu-waa-5": "post_exploitation",   # Reporting → Post-Exploitation (has report button)
    }
    if button_id in mapping:
        return mapping[button_id]
    raise dash.exceptions.PreventUpdate
...
if __name__ == "__main__":
    ...
    # Seed demo findings data if DB is empty (ensures trend charts show data)
    try:
        from cyber_range.services.findings_service import seed_demo_data as _seed
        _seed()
        print("[APP] ✅ Findings DB seeded / verified")
    except Exception as _se:
        print(f"[APP] ⚠ Could not seed findings: {_se}")
...
def submit_admin_login(_, username, password):
    ...
    perm_file = "/opt/vuln_intel/app/user_permissions.json"
    try:
