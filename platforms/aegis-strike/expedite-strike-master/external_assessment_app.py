...
def _handle_pentest_report(scan_id: str):
    ...
    try:
        ...
        # Enterprise PDF (if exploit chain data available)
        if _exploit_db_data and _exploit_db_data.get("results"):
            try:
                from cyber_range.exploit_db.report_generator import generate_executive_report
                _tmp_dir  = _tmp.mkdtemp()
                _pdf_path = os.path.join(_tmp_dir, fname)
                generate_executive_report(_exploit_db_data, _pdf_path)
                return send_file(_pdf_path, mimetype="application/pdf",
                                 as_attachment=True, download_name=fname)
            except Exception:
                pass  # Fall through to standard PDF

        # Standard PTES PDF  — wrap _flat in the schema generate_pdf expects:
        #   { "target": str, "targets": list, "timestamp": str,
        #     "phases_run": list, "data": { TOOL: tool_data_dict } }
...
if __name__ == "__main__":
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════════╗
║   Ægis  —  External Assessment  (Standalone)                   ║
╠══════════════════════════════════════════════════════════════════╣
