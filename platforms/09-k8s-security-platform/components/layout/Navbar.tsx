...
const NAV = [{ label: "Home", href: "/" }, { label: "Audit", href: "/scan" }, { label: "Dashboard", href: "/dashboard" }];
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b"
      style={{ borderColor: "var(--border)", background: "rgba(7,10,15,0.88)", backdropFilter: "blur(12px)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-sm font-bold text-white">K8s Security</div>
              <div className="text-[10px]" style={{ color: "var(--muted)" }}>Platform 9 of 34</div>
            </div>
          </Link>
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map(l => (
              <Link key={l.href} href={l.href}
                ...
                style={{ color: pathname === l.href ? "var(--primary)" : "var(--muted)", background: pathname === l.href ? "rgba(129,140,248,0.08)" : "transparent" }}>
                {l.label}
              </Link>
            ))}
          </nav>
          <div className="hidden md:flex">
            <Link href="/scan"
              ...
              <Shield className="w-3.5 h-3.5" /> Audit Cluster
            </Link>
          </div>
