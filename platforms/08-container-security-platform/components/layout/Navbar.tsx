...
import { Container, Menu, X, ChevronRight } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b"
      style={{ borderColor: "var(--border)", background: "rgba(6,11,20,0.88)", backdropFilter: "blur(12px)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              style={{ background: "rgba(56,189,248,0.12)", border: "1px solid rgba(56,189,248,0.3)" }}>
              <Container className="w-4 h-4" style={{ color: "var(--primary)" }} />
            </div>
            <div>
              <div className="text-sm font-bold text-white">Container Security</div>
              <div className="text-[10px]" style={{ color: "var(--muted)" }}>Platform 8 of 34</div>
            </div>
          </Link>
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map(l => (
              <Link key={l.href} href={l.href}
                ...
                style={{ color: pathname === l.href ? "var(--primary)" : "var(--muted)", background: pathname === l.href ? "rgba(56,189,248,0.08)" : "transparent" }}>
                {l.label}
              </Link>
            ))}
          </nav>
          <div className="hidden md:flex">
            <Link href="/scan"
              ...
              <Container className="w-3.5 h-3.5" /> Scan Image
            </Link>
          </div>
