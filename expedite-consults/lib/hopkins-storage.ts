...
const STORAGE_KEYS = {
  ...
  HOPKINS_BUILDINGS: "hopkinssync_jhu_buildings_v10",
  HOPKINS_CIRCLES: "hopkinssync_jhu_circles_v10",
  HOPKINS_SHUTTLES: "hopkinssync_jhu_shuttles_v10",
  HOPKINS_PARKING: "hopkinssync_jhu_parking_v10",
  HOPKINS_SAFETY: "hopkinssync_jhu_safety_v10",
  HOPKINS_SCAVENGER: "hopkinssync_jhu_scavenger_v10",
...
export function loadCampusWeather(): CampusWeatherReport {
  const loaded = safeLoad<CampusWeatherReport>(STORAGE_KEYS.CAMPUS_WEATHER, initialHopkinsMainWeather);
  return {
    ...initialHopkinsMainWeather,
    ...(loaded || {}),
    hourlyForecast: Array.isArray(loaded?.hourlyForecast) && loaded.hourlyForecast.length > 0 ? loaded.hourlyForecast : initialHopkinsMainWeather.hourlyForecast,
    dailyForecast: Array.isArray(loaded?.dailyForecast) && loaded.dailyForecast.length > 0 ? loaded.dailyForecast : initialHopkinsMainWeather.dailyForecast,
    activeAlerts: Array.isArray(loaded?.activeAlerts) ? loaded.activeAlerts : initialHopkinsMainWeather.activeAlerts,
    clothingRecommendation: loaded?.clothingRecommendation || initialHopkinsMainWeather.clothingRecommendation,
    conditionIcon: loaded?.conditionIcon || initialHopkinsMainWeather.conditionIcon,
    radarImageUrl: loaded?.radarImageUrl || initialHopkinsMainWeather.radarImageUrl,
  };
}
...
// ─────────────────────────────────────────────────────────────
export function loadHopkinsBuildings(): HopkinsBuilding[] {
  const loaded = safeLoad<HopkinsBuilding[]>(STORAGE_KEYS.HOPKINS_BUILDINGS, initialHopkinsBuildings);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsBuildings;
}
export function saveHopkinsBuildings(buildings: HopkinsBuilding[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_BUILDINGS, buildings);
}

export function loadHopkinsCircles(): LocationCircle[] {
  const loaded = safeLoad<LocationCircle[]>(STORAGE_KEYS.HOPKINS_CIRCLES, initialHopkinsCircles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsCircles;
}
export function saveHopkinsCircles(circles: LocationCircle[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_CIRCLES, circles);
}

export function loadHopkinsShuttles(): HopkinsShuttle[] {
  const loaded = safeLoad<HopkinsShuttle[]>(STORAGE_KEYS.HOPKINS_SHUTTLES, initialHopkinsShuttles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsShuttles;
}
export function saveHopkinsShuttles(shuttles: HopkinsShuttle[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_SHUTTLES, shuttles);
}

export function loadHopkinsParking(): HopkinsParkingGarage[] {
  const loaded = safeLoad<HopkinsParkingGarage[]>(STORAGE_KEYS.HOPKINS_PARKING, initialHopkinsParking);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsParking;
}
export function saveHopkinsParking(parking: HopkinsParkingGarage[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_PARKING, parking);
}

export function loadHopkinsSafetyBeacons(): SafetyBeacon[] {
  const loaded = safeLoad<SafetyBeacon[]>(STORAGE_KEYS.HOPKINS_SAFETY, initialHopkinsSafetyBeacons);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsSafetyBeacons;
}
export function saveHopkinsSafetyBeacons(beacons: SafetyBeacon[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_SAFETY, beacons);
}

export function loadHopkinsScavengerCheckpoints(): ScavengerHuntCheckpoint[] {
  const loaded = safeLoad<ScavengerHuntCheckpoint[]>(STORAGE_KEYS.HOPKINS_SCAVENGER, initialHopkinsScavengerCheckpoints);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialHopkinsScavengerCheckpoints;
}
export function saveHopkinsScavengerCheckpoints(checkpoints: ScavengerHuntCheckpoint[]): void {
  safeSave(STORAGE_KEYS.HOPKINS_SCAVENGER, checkpoints);
}
