...
import { ScheduleEvent } from '../types/connect';

interface AxiomScheduleViewProps {
...
export const MOCK_EVENTS: ScheduleEvent[] = [
  {
    ...
    session_id: 'AXM-492-831',
    session_link: 'https://session.axiomconnect.com/AXM-492-831',
  ...
  },
  {
    ...
    session_id: 'AXM-782-104',
    session_link: 'https://session.axiomconnect.com/AXM-782-104',
  ...
  {
    id: 'ev-3',
    title: 'Weekly SRE & Relay Delivery Infrastructure Standup',
    description: 'Reviewing Rspamd DKIM, SPF alignment, and DMARC quarantine reports across domains.',
    session_id: 'AXM-319-902',
    session_link: 'https://session.axiomconnect.com/AXM-319-902',
...
export const AxiomScheduleView: React.FC<AxiomScheduleViewProps> = ({ onJoinSession }) => {
  const [events, setEvents] = useState<ScheduleEvent[]>(MOCK_EVENTS);
  const [selectedEvent, setSelectedEvent] = useState<ScheduleEvent | null>(MOCK_EVENTS[0]);
  ...
  const handleCreateSession = () => {
    if (!newTitle) return;
    const sessionId = `AXM-${Math.floor(100 + Math.random() * 900)}-${Math.floor(100 + Math.random() * 900)}`;
    const newEv: ScheduleEvent = {
      id: `ev-${Date.now()}`,
      title: newTitle,
      session_id: sessionId,
      session_link: `https://session.axiomconnect.com/${sessionId}`,
    ...
    <div className="flex-1 flex flex-col h-[calc(100vh-4rem)] overflow-hidden bg-white">
      ...
      <div className="h-12 bg-slate-50 border-b border-slate-200 px-4 flex items-center justify-between shrink-0 select-none">
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => onJoinSession('AXM-492-831', 'Instant Telepresence Session')}
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {idx === 2 && (
                    ...
                    >
                      ...
                      <p className="text-xs font-bold leading-tight">SRE Standup: Relay Transport</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-purple-600 text-white shadow-md">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold">Axiom Session Room</span>
                    </div>
                  ...
                  <p className="text-xs text-purple-100 mb-3">
                    Multi-party encrypted WebRTC room with live Pulse AI transcription & screen sharing.
                  </p>
                  <button
                    onClick={() => onJoinSession(selectedEvent.session_id!, selectedEvent.title)}
                  ...
                  >
                    ...
                    <span>Enter Telepresence Session</span>
                  </button>
                </div>
              )}
              ...
              <div className="space-y-2 text-xs text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>https://session.axiomconnect.com/{selectedEvent.session_id}</span>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-violet-50 border border-violet-200 text-xs text-violet-900">
                <div className="flex items-center gap-1.5 font-bold mb-1 text-violet-950">
                  ...
                  <span>Pulse AI Availability Matrix</span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-[460px] p-5 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-slate-900">Schedule Axiom Session</span>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="font-bold text-slate-600 block mb-1">Session Title</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 flex items-center gap-2">
                ...
                <span className="text-purple-900 font-medium text-[11px]">
                  Automatically generate unique Axiom Session telepresence link
                </span>
              </div>
            </div>
