...
import {
  ...
  Send, MoreVertical, Maximize2, Radio
} from 'lucide-react';
import { MeetingParticipant, MeetingDecision, MeetingActionItem } from '../types/connect';
...
const INITIAL_PARTICIPANTS: MeetingParticipant[] = [
...
export const VideoMeetingRoom: React.FC<VideoMeetingRoomProps> = ({
  ...
  const [participants, setParticipants] = useState<MeetingParticipant[]>(INITIAL_PARTICIPANTS);
  const [isMuted, setIsMuted] = useState(false);
  const [isVideoOff, setIsVideoOff] = useState(false);
  const [isScreenSharing, setIsScreenSharing] = useState(false);
  const [activeTab, setActiveTab] = useState<'ai_assistant' | 'chat' | 'people'>('ai_assistant');
  ...
  const [decisions, setDecisions] = useState<MeetingDecision[]>([
    { id: 'd-1', text: 'Approved Section 4 Multi-Agent RL mitigation loop architecture for committee submission.' },
  ...
  const [actionItems, setActionItems] = useState<MeetingActionItem[]>([
    ...
    { id: 'a-2', assignee: 'Sarah Chen', task: 'Upload PCAP packet traces to Axiom Drive folder', is_done: false },
  ...
  return (
    <div className="flex-1 bg-slate-950 text-white flex flex-col h-[calc(100vh-4rem)] overflow-hidden select-none">
      ...
      <div className="h-14 bg-slate-900 border-b border-slate-800 px-5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">Encrypted WebRTC SFU • 4 Participants</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setActiveTab(activeTab === 'ai_assistant' ? 'chat' : 'ai_assistant')}
      ...
      {/* Main Center Video Grid + Right Side Panel */}
      <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 flex flex-col items-center justify-center relative">
                    ...
                    {p.isScreenSharing && (
                      <div className="absolute inset-4 bg-slate-900/90 rounded-xl border border-slate-700 flex flex-col items-center justify-center p-4 text-center">
                        ...
                        <p className="text-xs font-bold text-slate-200">Sharing Screen: Defense_Loops_Benchmarks.pdf</p>
                      </div>
                    )}
                  </div>
                ) : (
        ...
        {/* Right Drawer: AI Assistant / Meeting Chat / People */}
