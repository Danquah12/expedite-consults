...
import {
  Hash, Lock, Video, Plus, Send, Smile, Paperclip,
  Users, MoreVertical, MessageSquare, ShieldCheck
} from 'lucide-react';
import { NexusChannel, NexusMessage, UserPresence } from '../types/connect';

interface AxiomNexusViewProps {
  onLaunchSession: (sessionId: string, title: string) => void;
}

const MOCK_CHANNELS: NexusChannel[] = [
  ...
  { id: 'ch-3', name: 'engineering', topic: 'Platform SRE, Relay Engine & LMTP' },
...
const INITIAL_MESSAGES: Record<string, NexusMessage[]> = {
  'ch-2': [
    {
      id: 'm-1',
    ...
    },
    {
      id: 'm-4',
      sender: MOCK_COLLEAGUES[0],
      content: 'Let’s enter an Axiom Session to finalize the committee presentation slide deck.',
      timestamp: '10:25 AM',
      sessionCard: {
        sessionId: 'AXM-492-831',
        title: 'Autonomous Cyber Defense Loops: Committee Telepresence',
        time: 'Live Now (3 Participants)',
        link: 'https://session.axiomconnect.com/AXM-492-831'
      }
    }
  ]
};

export const AxiomNexusView: React.FC<AxiomNexusViewProps> = ({ onLaunchSession }) => {
  const [activeChannelId, setActiveChannelId] = useState<string>('ch-2');
  const [messages, setMessages] = useState<Record<string, NexusMessage[]>>(INITIAL_MESSAGES);
  ...
  const handleStartInstantSession = () => {
    onLaunchSession('AXM-492-831', `#${currentChannel.name} Telepresence`);
  };

  return (
    <div className="flex-1 flex h-[calc(100vh-4rem)] overflow-hidden bg-white">
      {/* Nexus Channel & Direct Sync Sidebar */}
      <div className="w-64 bg-slate-50 border-r border-slate-200 p-3 shrink-0 flex flex-col justify-between select-none">
        <div className="space-y-4">
          <div className="px-2 flex items-center justify-between">
            <div>
              <h2 className="text-xs font-bold text-slate-900">Axiom Nexus</h2>
              <span className="text-[10px] text-slate-500 font-medium">Team Collaboration & Sync</span>
            </div>
          ...
          <div className="space-y-1">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2">Workspaces</p>
          ...
          {/* Direct Syncs */}
          <div className="space-y-1">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2">Direct Syncs</p>
        ...
        <div className="p-2.5 rounded-xl bg-purple-50 border border-purple-200 text-xs">
          <div className="flex items-center gap-1.5 font-bold text-purple-950 mb-1">
            ...
            <span>Axiom Session Telepresence</span>
          </div>
          <button
          ...
          >
            ...
            <span>Launch #{currentChannel.name} Session</span>
          </button>
        </div>
      </div>
                  ...
                  {msg.sessionCard && (
                    <div className="mt-3 p-3 rounded-xl bg-purple-600 text-white flex items-center justify-between shadow-md">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-bold">{msg.sessionCard.title}</p>
                          <p className="text-[11px] text-purple-200">{msg.sessionCard.time}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => onLaunchSession(msg.sessionCard!.sessionId, msg.sessionCard!.title)}
        ...
        <div className="p-4 border-t border-slate-200 bg-white">
          <div className="rounded-2xl border border-slate-300 p-2.5 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition">
            <textarea
              ...
              rows={2}
              placeholder={`Sync in #${currentChannel.name}...`}
