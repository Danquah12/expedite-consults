...
const STORAGE_KEYS = {
  ...
  SALISBURY_BUILDINGS: "seagullsync_su_buildings_v10",
  SALISBURY_CIRCLES: "seagullsync_su_circles_v10",
  SALISBURY_SHUTTLES: "seagullsync_su_shuttles_v10",
  SALISBURY_PARKING: "seagullsync_su_parking_v10",
  SALISBURY_SAFETY: "seagullsync_su_safety_v10",
  SALISBURY_SCAVENGER: "seagullsync_su_scavenger_v10",
...
export function loadCampusWeather(): CampusWeatherReport {
  const loaded = safeLoad<CampusWeatherReport>(STORAGE_KEYS.CAMPUS_WEATHER, initialSalisburyMainWeather);
  return {
    ...initialSalisburyMainWeather,
    ...(loaded || {}),
    hourlyForecast: Array.isArray(loaded?.hourlyForecast) && loaded.hourlyForecast.length > 0 ? loaded.hourlyForecast : initialSalisburyMainWeather.hourlyForecast,
    dailyForecast: Array.isArray(loaded?.dailyForecast) && loaded.dailyForecast.length > 0 ? loaded.dailyForecast : initialSalisburyMainWeather.dailyForecast,
    activeAlerts: Array.isArray(loaded?.activeAlerts) ? loaded.activeAlerts : initialSalisburyMainWeather.activeAlerts,
    clothingRecommendation: loaded?.clothingRecommendation || initialSalisburyMainWeather.clothingRecommendation,
    conditionIcon: loaded?.conditionIcon || initialSalisburyMainWeather.conditionIcon,
    radarImageUrl: loaded?.radarImageUrl || initialSalisburyMainWeather.radarImageUrl,
  };
}
...
// ─────────────────────────────────────────────────────────────
export function loadSalisburyBuildings(): SalisburyBuilding[] {
  const loaded = safeLoad<SalisburyBuilding[]>(STORAGE_KEYS.SALISBURY_BUILDINGS, initialSalisburyBuildings);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburyBuildings;
}
export function saveSalisburyBuildings(buildings: SalisburyBuilding[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_BUILDINGS, buildings);
}

export function loadSalisburyCircles(): LocationCircle[] {
  const loaded = safeLoad<LocationCircle[]>(STORAGE_KEYS.SALISBURY_CIRCLES, initialSalisburyCircles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburyCircles;
}
export function saveSalisburyCircles(circles: LocationCircle[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_CIRCLES, circles);
}

export function loadSalisburyShuttles(): SalisburyShuttle[] {
  const loaded = safeLoad<SalisburyShuttle[]>(STORAGE_KEYS.SALISBURY_SHUTTLES, initialSalisburyShuttles);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburyShuttles;
}
export function saveSalisburyShuttles(shuttles: SalisburyShuttle[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_SHUTTLES, shuttles);
}

export function loadSalisburyParking(): SalisburyParkingGarage[] {
  const loaded = safeLoad<SalisburyParkingGarage[]>(STORAGE_KEYS.SALISBURY_PARKING, initialSalisburyParking);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburyParking;
}
export function saveSalisburyParking(parking: SalisburyParkingGarage[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_PARKING, parking);
}

export function loadSalisburySafetyBeacons(): SafetyBeacon[] {
  const loaded = safeLoad<SafetyBeacon[]>(STORAGE_KEYS.SALISBURY_SAFETY, initialSalisburySafetyBeacons);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburySafetyBeacons;
}
export function saveSalisburySafetyBeacons(beacons: SafetyBeacon[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_SAFETY, beacons);
}

export function loadSalisburyScavengerCheckpoints(): ScavengerHuntCheckpoint[] {
  const loaded = safeLoad<ScavengerHuntCheckpoint[]>(STORAGE_KEYS.SALISBURY_SCAVENGER, initialSalisburyScavengerCheckpoints);
  return Array.isArray(loaded) && loaded.length > 0 ? loaded : initialSalisburyScavengerCheckpoints;
}
export function saveSalisburyScavengerCheckpoints(checkpoints: ScavengerHuntCheckpoint[]): void {
  safeSave(STORAGE_KEYS.SALISBURY_SCAVENGER, checkpoints);
}
